...
<div class="container mt-5">
    ...
    <div class="row mt-3">
        <div class="col-md-12">
            <div class="box-showing">
                <div class="tab-header text-center">
                    <h4>TEMPS D' ATTENDRE </h4>
                </div>
                <div class="row mt-3">
                    <div class="col-md-12">
                        <div class="text-center">
                            <h1 id="countdown_text">0</h1>
                            <!-- <h3>Temps restant</h3> -->
                                ...
                                <div class="row">
                                    <div class="col-md-9 col-sm-6">
                                        <div class="progress blue">
                                            ...
                                            <div class="progress-value" id="countdown_new_text">100%</div>
                                        </div>
                                    </div>
                                </div>
                            </div> -->
    ...
    <div class="row mt-3 mb-5">
        <div class="col-md-12 mt-4">
            <div class="box-showing">
                <div class="tab-header text-center">
                    <h4>Reglage du backwash CountDown</h4>
                </div>
                <div class="row mt-4">
                    <div class="col-md-4">
                        <h3>Countdown Time : (Min) </h3>
                    </div>

                    <div class="col-md-8 ">
                        <div class="float-end">
                            <div class="input-group mb-3">
                                <span class="input-group-text" id="basic-addon3" onclick="update_time_countdown('minus');"><i class="fa fa-minus fa-lg"></i></span>
                                <input type="text" aria-label="Username" id="time_countdown" value="<?= $info[0]->backwash_countdown ?>" style=" width: 70px; text-align : center">
                                <span class="input-group-text" id="basic-addon4" onclick="update_time_countdown('plus');"><i class="fa fa-plus fa-lg"></i></span>
                            </div>
                        </div>
                    </div>
                </div>
...
<!-- online set -->
<script>
    setInterval(load_counter_backwash, 1000);
    $(function() {
        set_state_day();
        load_counter_backwash();

    });

    function load_counter_backwash() {
        $.getJSON("<?= base_url('Read_status/read_counter_backwash') ?>", function(data) {
            // var time = parseInt(data);
            // if (time > 60) {
            //     var div_time = time / 60;
            //     var mod_time = time % 60;
            //     var data_time_set = div_time.toString() + ' : ' + modurus_time.toString();
            //     $('#countdown_text').html(data_time_set);
            // } else {
                $('#countdown_text').html(data);
            // }
        });
    }
    ...
    function update_time_backwash(status) {
        ...
        $.post("<?= base_url('admin/backwash/update_adjust_time') ?>", {
            ...
            update_time_backwash_online();
        });
    }

    function update_time_backwash_online() {

        var path_server = $('#path_server').val() + 'api/Receive_setting/update_time_backwash';
        // console.log(path_server);
        $.post(path_server, {
            adjust_time: $('#adjust_time').val(),
            machine_code: $('#machine_code').val()
        }, function() {
    ...
    function update_time_countdown(status) {
        var new_value = 0
        if (status == 'minus') {
            var time_countdown = $('#time_countdown').val();
            if (parseInt(time_countdown) != 0) {
                new_value = parseInt(time_countdown) - 1
            }
        ...
        $('#time_countdown').val(new_value);
        $.post("<?= base_url('admin/backwash/update_time_countdown') ?>", {
            new_value: new_value
        }, function() {
            update_time_countdown_online();
        })

    }

    function update_time_countdown_online() {
        var path_server = $('#path_server').val() + 'api/Receive_setting/update_time_countdown';
        $.post(path_server, {
            time_countdown: $('#time_countdown').val(),
            machine_code: $('#machine_code').val()
        }, function() {
